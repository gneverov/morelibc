...
static inline int poll_file_fd(struct poll_file *file, int flags) {
    return vfs_replace(-1, &file->base, flags);
}
...
// helper macro
#define POLL_CHECK(flags, ret, file, events, pxTicksToWait) \
    (!(flags & FNONBLOCK) && (ret < 0) && (errno == EAGAIN) && ((ret = poll_file_wait(file, events, pxTicksToWait)) >= 0))
