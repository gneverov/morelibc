...
// for use in the flags argument to getnameinfo():
#define NI_NOFQDN 0x01                  // Only the nodename portion of the FQDN is returned for local hosts.
#define NI_NUMERICHOST 0x02             // The numeric form of the node's address is returned instead of its name.
#define NI_NAMEREQD 0x04                // Return an error if the node's name cannot be located in the database.
#define NI_NUMERICSERV 0x08             // The numeric form of the service address is returned instead of its name.
#define NI_NUMERICSCOPE 0x10            // For IPv6 addresses, the numeric form of the scope identifier is returned instead of its name.
#define NI_DGRAM 0x20                   // Indicates that the service is a datagram service (SOCK_DGRAM).
